...
describe("Loop Completion Integration Tests", () => {
    ...
    describe("Butterfly-Loop Interaction", () => {
        ...
        it("should handle single butterfly color switching", () => {
            ...
            lineDrawer.on("loopAreaCompleted", (loopGraphics: any) => {
                ...
                const capturedButterflies = [singleButterfly].filter((b) =>
                    b.isHit(loopGraphics),
                );
        ...
        it("should validate successful loop conditions", () => {
            ...
            testCases.forEach((testCase) => {
                ...
                lineDrawer.on("loopAreaCompleted", (loopGraphics: any) => {
                    ...
                    testCase.butterflies.forEach((b) =>
                        b.setMockCaptured(true),
                    );
                    capturedButterflies.push(
                        ...testCase.butterflies.filter((b) =>
                            b.isHit(loopGraphics),
                        ),
                    );
                    ...
                    const colors = Array.from(
                        new Set(capturedButterflies.map((b) => b.color)),
                    );

                    if (
                        (colors.length >= 3 &&
                            colors.length === capturedButterflies.length) ||
    ...
    describe("Edge Cases and Error Handling", () => {
        it("should handle empty loops (no butterflies captured)", () => {
            ...
            lineDrawer.on("loopAreaCompleted", (loopGraphics: any) => {
                loopCompleted = true;
                capturedCount = butterflies.filter((b) =>
                    b.isHit(loopGraphics),
                ).length;
            });
    ...
    describe("Performance and Memory Management", () => {
        ...
        it("should handle large numbers of butterflies efficiently", () => {
            ...
            const manyButterflies = Array.from(
                { length: 100 },
                (_, i) =>
                    new TestButterfly(
                        (i % 20) * 10,
