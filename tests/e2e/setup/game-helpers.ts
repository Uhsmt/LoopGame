...
export class GameHelper {
    ...
    constructor(page: Page) {
        ...
        this.gameContainer = page.locator(
            "#game-container, .game-container, body",
        );
    }
            ...
            () => {
                ...
                return (
                    document.querySelector(
                        '.score, .timer, [data-testid="score"], [data-testid="timer"]',
                    ) !== null || (window as any).gameState === "gameplay"
                );
            },
            { timeout: 5000 },
        );
    ...
    async getRemainingTime(): Promise<number> {
        const timeText = await this.page
            .locator('.timer, [data-testid="timer"]')
            .first()
            .textContent();
    ...
    async waitForGameEnd() {
        await this.page.waitForFunction(
            () => {
                return (
                    (window as any).gameState === "result" ||
                    document.querySelector(
                        '.game-over, .result, [data-testid="game-over"]',
                    ) !== null
                );
            },
            { timeout: 70000 },
        ); // 70 seconds max (longer than game duration)
    }
    ...
    async butterflyCount(): Promise<number> {
        return await this.page.evaluate(() => {
            ...
            const butterflies = document.querySelectorAll(
                '.butterfly, [data-testid="butterfly"]',
            );
    ...
    async dismissMobileWarning() {
        const mobileWarning = this.page
            .locator("text=PC only, text=Desktop only")
            .first();
